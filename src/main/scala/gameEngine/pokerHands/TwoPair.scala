...
case object TwoPair extends PokerHandFactory(TwoPairRank) {
  ...
  override def makeHand(cards: List[Card]) = new TwoPair(cards)
  ...
  /** Tests if given cards list make a two pair hand.
    ...
    */
  override def isMadeUpOf(cards: List[Card]): Boolean = PokerHandFactory.countAndFilterRanks(cards, _ > 1).length == 2
}
...
case class TwoPair private(override val cards: List[Card]) extends PokerHand(TwoPairRank, cards) {
  ...
  override protected def isStrongerWithinRank(other: PokerHand): Boolean = {
    ...
    val thisPairRanks: List[CardRank] = PokerHandFactory.countAndFilterRanks(this.cards, _ == 2).sortWith(_ > _)
    val otherPairRanks: List[CardRank] = PokerHandFactory.countAndFilterRanks(other.cards, _ == 2).sortWith(_ > _)
    ...
    val thisKickerRank = PokerHandFactory.countAndFilterRanks(this.cards, _ == 1).head
    val otherKickerRank = PokerHandFactory.countAndFilterRanks(other.cards, _ == 1).head
